...
## 21. `subPremise` vs `subpremise` casing

**What happened**: Queried `getAvailableAddresses { subPremise }` — got validation error.

**Fix**: Field is `subpremise` (all lowercase), not `subPremise` (camelCase). The `DefaultAddress` type uses lowercase.

---

## 22. `updateConsumerDefaultAddress` is the address change mutation

**What happened**: Probed 8 mutation name candidates. `updateDefaultAddress` gave "Did you mean `updateConsumerDefaultAddress`?". `updateConsumerAddress` revealed 5 related mutations: `editConsumerAddress`, `addConsumerAddress`, `removeConsumerAddress`, `editConsumerAddressV2`, `editConsumerAddressV3`.

**Usage**: `mutation { updateConsumerDefaultAddress(defaultAddressId: "ID") { ... } }` — single required arg, returns `Consumer` type. Address ID must come from `getAvailableAddresses`. Change is immediate and account-level.
