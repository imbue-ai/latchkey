...
>(BOWEI) pin to a hash, clone it down. use the latest hash from bowei/doordash-clean.
...
>(BOWEI) yes this. the or their agent will be able to figure out how to download.
...
>(BOWEI) no, force them to clone down the repo
...
>(BOWEI) you also forgot, make them clone down doordash mcp
...
>(BOWEI) no, you got the reason wrong. we only provide this for the full graphql schema dump - as REFERENCE MATERIAL, not intended to be used. Latchkey auth story is much better.
...
use latchkey-doordash-agent-proto-skill

2. **Ship both latchkey+curl AND doordash-mcp?** Or just one path?

dont ship anything just rec docs
