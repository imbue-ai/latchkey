...
- `LATCHKEY_LATCKEY_DIAGNOSTIC_HEADERS`: if set, Latchkey populates some special headers with useful diagnostic information when calling curl. This currently includes `X-Latchkey-Matched-Service`, indicating the servie that Latchkey matched the request to and injected credentials for.
